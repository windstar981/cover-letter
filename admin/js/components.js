...
CalendarInline.prototype.renderWidget = function (date, month, day) {
  ...
  this.widget.querySelector(".calendar-widget__month").textContent =
    calendarMonth[month];
  this.widget.querySelector(".calendar-widget__weekday").textContent =
    this.weekDay[day];
};
...
CalendarInline.prototype.createEvents = function (dObj, dStr, fp, day) {
  ...
  if (
    new Date().getTime() - 24 * 2 * 3600 * 1000 >
      eventDate.getTime() - 24 * 1 * 3600 * 1000 &&
  ...
  if (
    new Date().getTime() + 24 * 5 * 3600 * 1000 <
      eventDate.getTime() + 24 * 1 * 3600 * 1000 &&
...
function chart() {
  ...
  const revenueLineChartOptions = {
    ...
    fill: {
      ...
      colors: [
        themeStyle("--grey-color-4", "--black-color-3"),
        themeStyle("rgba(255, 138, 72, 0.5)", "rgba(255, 138, 72, 0.3)", false),
      ],
      gradient: {
        type: "horizontal",
        gradientToColors: [
          themeStyle("--grey-color-4", "--black-color-3"),
          themeStyle("rgba(255, 61, 87, 0.5)", "rgba(255, 61, 87, 0.3)", false),
        ],
...
function Chat() {
  ...
  this.userAddToggleElement = document.querySelector(
    '[data-toggle="users-add-list"]'
  );
  this.usersNew = document.querySelectorAll(
    ".chat-users__add-content .chat-users__item"
  );
  this.users = document.querySelectorAll(
    ".chat-users__content .chat-users__item"
  );

  if (this.chat) {
    this.buttonToggleUsers.addEventListener("click", (event) =>
      this.usersOpen(event)
    );
    this.detailsBackdrop.addEventListener("click", () => this.usersClose());
    this.userAddToggleElement.addEventListener("click", (event) =>
      this.userAddListToggle(event)
    );
  ...
  if (this.toggleButton[0]) {
    ...
    document.addEventListener("click", (event) => {
      if (
        ...
        if (
          this.toggleButton[this.getActiveElement].classList.contains(
            "active"
          ) &&
...
function SidebarPanel() {
  ...
  if (this.buttonToggleElement) {
    this.buttonToggleElement.addEventListener("click", (event) =>
...
ImageUpload.prototype.parseFile = function (file, input) {
  ...
  const list = inputElement
    .closest(".image-upload")
    .querySelector(".image-upload__list");
  const items = inputElement
    .closest(".image-upload")
    .querySelectorAll(".image-upload__item:not(.is-active)");
  ...
  items[0]
    .querySelector(".image-upload__preview-image")
    .addEventListener("click", (event) => ImageUpload.removeItem(event, items));
...
function Inbox() {
  ...
  this.detailsDismiss = document.querySelectorAll(
    ".inbox-details [data-dismiss]"
  );
      ...
      init() {
        ...
        items.forEach((item, index) => {
          const activeLink = item
            .querySelector(".order-tabs__link")
            .classList.contains("order-tabs__link--active");
...
function AddProduct() {
  ...
  this.mainSliderElement = this.element.querySelector(
    ".add-product__gallery-slider"
  );
  this.thumbsSliderElement = this.element.querySelector(
    ".add-product__thumbs-slider"
  );
  this.thumbsNavButton = this.element.querySelectorAll(
...
Checkboxes.prototype.events = function () {
  ...
  this.element.forEach((el) => {
    ...
    const inputs = document.querySelectorAll(
      `[data-checkbox="${checkboxAll.getAttribute("data-checkbox-all")}"]`
    );
...
Checkboxes.prototype.checkedAll = function (event) {
  ...
  const inputs = document.querySelectorAll(
    `[data-checkbox="${checkbox.getAttribute("data-checkbox-all")}"]`
  );
  const inputsChecked = [];
...
Checkboxes.prototype.checkedInput = function (event) {
  ...
  const checkboxAll = document.querySelector(
    `[data-checkbox-all="${input.getAttribute("data-checkbox")}"]`
  );
  const inputs = document.querySelectorAll(
    `[data-checkbox="${checkboxAll.getAttribute("data-checkbox-all")}"]`
  );
...
SwitcherGroup.prototype.toggle = function (event) {
  ...
  switcher
    .querySelectorAll(".switcher-button__item")
...
InputSelects.prototype.scrollbar = function (event) {
  const dropdown = event.currentTarget.parentNode.querySelector(
    ...
    $(element)
      .select2({
        ...
        templateSelection: (data) => {
          return $(
            `<span>${data.text}<span class="select2-selection__choice__remove"><svg class="icon-icon-cross"><use xlink:href="#icon-cross"></use></svg></span></span>`
          );
        },
        placeholder: $(element.getAttribute("data-placeholder")),
        dropdownParent: $(element.parentNode),
...
function DropdownSelect(dropdownElement) {
  ...
  this.input = this.dropdown.parentNode.querySelector(
